...
(defn- dispatch-op [args]
  ...
  (let [[event-str rest-args] (partition-dispatch-args args)]
    ...
    (let [build-id    (build-id-from-args rest-args)
      ...
      (try
        (cond
          ;; --trace: sync dispatch, capture :dispatch-id (auto-
          ;; generated by re-frame core under rf-3p7 / commit af024c3),
          ;; wait past trace-debounce, then collect the matching epoch
          ;; in a second eval. We do the wait bash-side because
          ;; dispatch-and-collect (the Promise version) doesn't
          ;; round-trip cleanly through cljs-eval.
          trace?
